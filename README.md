...
5. Locate and select the `.vsix` package:
   * **If installed via the Debian package:** `/usr/share/omniflux/extensions/omniflux-support/omniflux-support-1.0.2.vsix`
   * **If cloned from the Git repository:** `editors/vscode/omniflux-support-1.0.2.vsix` inside the cloned directory.
6. Click **Install**.
