// .omniflux_cache/examples/03_default_params.js
var fs = require("fs");
...
async function greet(name = "Guest", greeting = "Hello") {
  print(greeting + ", " + name + "!");
}
async function create_user(username, role = "member", active = true) {
  print("User: %s (Role: %s, Active: %s)", username, role, active);
}
async function __on_start() {
  print("=== Testing Default Parameters ===");
  print("Calling await greet() with no arguments:");
  await greet();
  print("\nCalling await greet() with only one argument:");
  await greet("Alice");
  print("\nCalling await greet() with all arguments:");
  await greet("Bob", "Good morning");
  print("\n----------------------------------------");
  print("Calling await create_user with only required argument:");
  await create_user("charlie");
  print("\nCalling await create_user overriding the role:");
  await create_user("david", "admin");
  print("\nCalling await create_user overriding all arguments:");
  await create_user("eve", "moderator", false);
}
global.greet = greet;
