...
async function get_timestamp() {
  let ts = time();
  let year = dateyear(ts);
  let month = datemonth(ts);
  let day = dateday(ts);
  let hours = datehour(ts);
  let minutes = dateminute(ts);
  let seconds = datesecond(ts);
